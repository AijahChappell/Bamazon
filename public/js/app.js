    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == XMLHttpRequest.DONE) {
            ...
            products.forEach(function(product) {
                ...
                productDiv.innerHTML = '<h3>${productName}</h3><h4><i class = "${icon}" style="color:$(color)"></i></h4><h4>${productPrice)</h4><h4>${productPrice}</h4><label for = "quantity">Quantity:</label><input type = "text" size = "1" id = "${productName}quantity" />${button}';
...
$(document).on("click", ".submit", function(product) {
